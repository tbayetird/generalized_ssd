...
# from config_models import ssd300_pirogues_mer ,ssd300_road
...
def inference_on_image(model_config,image_path,conf_thresh=0.5):
    '''
...
def truncate_inference_on_video(model_config,
                       video_path,
                       truncate_low = 4000,
                       truncate_up=4500,
                       Display=True,
    ...
    vs = cv2.VideoCapture(video_path)
    (orig_images,input_images,out)=vd.handleTruncatedVideoStreams(vs,save_dir,video_name,img_width,img_height,[truncate_low,truncate_up])
    ...
    ### PREDICTING
    ##################################
    predName='trunc_pred_'+video_name
    print("[INFO] Predicting results on video stream  ")
    y_pred = model.predict(input_images)
    y_pred_thresh = [y_pred[k][y_pred[k,:,1] > confidence_threshold] for k in range(y_pred.shape[0])]
...
# truncate_inference_on_video(ssd300_pirogues_mer,'D:\\datas\\pirogues-mer\\videos_parrot\\Disco_0\\Disco_0_2.mp4',change_save_dir=True,tracking=True,half_bottom_only=True)
# inference_on_video(ssd300_road,'D:\\workspace\\Keras\\ssd_keras\\data\\Frogger3.mp4',tracking=True)
# inference_on_big_video(ssd300_pirogues_mer,'D:\\datas\\pirogues-mer\\videos_parrot\\Disco_0.mp4','full_stitched_video.mp4',500,0.3,tracking=True)
